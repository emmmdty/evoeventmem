...
> 六个独立开关：证据策略（constrained vs provenance-only）、时序源、图源、路由（规则 vs 强制）、权重策略、预算。每个开关只改变一个变量，其他全部固定，然后看决策差异（selection/exclusion/ranking）。这在受控夹具上验证"开关确实改变了决策"，在真实数据集上验证"改变决策是否影响结果"。
...
> 检索路径是纯数据库操作（pgvector HNSW），零 LLM 调用，毫秒级。写入路径的成本在 LLM 提取（可选规则提取器零成本）。诚实地说：精确的 p95 延迟还没测完，这是我要补的数据。**（诚实承认未完成项，不编数据）**
...
| LongMemEval 跑批 | 🚧 服务器运行中（~40%） | 讲流程，**不讲结果** |
| LoCoMo 跑批 | ⏳ 未开始 | 讲计划 |
| 6 因子消融 | ⏳ 等 base 完成 | 讲设计 |
| 最终分析报告 | ⏳ 等跑批完成 | 讲 pipeline |
| p95 延迟测量 | ⏳ 未做 | 讲为什么还没做 |

**面试纪律**：任何"提升 X%"的说法，必须来自跑批完成后的真实数据。在这之前，一律说"正在跑，流程已通，等数据"。
